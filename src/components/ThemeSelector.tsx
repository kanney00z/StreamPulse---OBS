...
export const ThemeSelector: React.FC<ThemeSelectorProps> = ({
  ...
  const [filter, setFilter] = useState<'multistream' | 'twitch' | 'comic' | '3d' | 'classic' | 'all'>('multistream');

  const countMultistream = CHAT_THEMES.filter((t) => t.isMultistream).length;
  ...
  const filteredThemes = CHAT_THEMES.filter((t) => {
    if (filter === 'multistream') return t.isMultistream;
    if (filter === 'twitch') return t.isTwitchGlow;
    if (filter === 'comic') return t.isComic;
    if (filter === '3d') return t.is3D;
    if (filter === 'classic') return !t.is3D && !t.isComic && !t.isTwitchGlow && !t.isMultistream;
    return true;
  });
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <div className="flex items-center gap-2">
          <span className="text-xs text-purple-300 font-medium flex items-center gap-1.5 bg-purple-500/10 px-3 py-1 rounded-full border border-purple-500/25 shadow-sm">
            <Sparkles className="w-3.5 h-3.5 text-purple-400" /> รวม {CHAT_THEMES.length} ธีม (ใหม่ Multistream Pill & Twitch Dark Glow!)
          </span>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <button
          type="button"
          onClick={() => setFilter('multistream')}
          className={`px-3.5 py-1.5 rounded-xl text-xs font-black transition-all flex items-center gap-1.5 cursor-pointer ${
            filter === 'multistream'
              ? 'bg-gradient-to-r from-fuchsia-600/40 via-pink-600/30 to-cyan-600/40 text-white border border-fuchsia-400/80 shadow-[0_0_18px_rgba(217,70,239,0.4)] ring-1 ring-fuchsia-400/60'
              : 'bg-slate-900/70 text-slate-300 border border-white/10 hover:border-fuchsia-500/40'
          }`}
        >
          <Sparkles className="w-3.5 h-3.5 text-pink-400 fill-pink-400" />
          <span>🌐 Multistream Pill ({countMultistream} ตามคลิปเป๊ะ!)</span>
          <span className="px-1.5 py-0.2 rounded text-[10px] bg-gradient-to-r from-pink-500 to-fuchsia-500 text-white font-black animate-pulse">
            NEW VIDEO
          </span>
        </button>
        ...
        >
          ...
          <span>💜 Twitch Role Glow ({countTwitch})</span>
        </button>
        ...
        >
          คลาสสิก & มินิมอล ({CHAT_THEMES.length - count3D - countComic - countTwitch - countMultistream})
        </button>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3.5">
        {filteredThemes.map((theme: ChatThemeConfig) => {
          ...
          const isMultistream = !!theme.isMultistream;
          ...
          return (
            <button
              ...
              className={`text-left p-3.5 rounded-2xl border transition-all duration-200 relative group flex flex-col justify-between overflow-hidden cursor-pointer ${
                isSelected
                  ? isMultistream
                    ? 'bg-slate-900 border-fuchsia-400 shadow-[0_0_24px_rgba(217,70,239,0.45)] ring-2 ring-fuchsia-400/60'
                    : isTwitch
              ...
              } ${theme.is3D || isComic || isTwitch || isMultistream ? 'hover:-translate-y-0.5' : ''}`}
            >
              ...
              <div className="flex items-center justify-between w-full mb-2">
                ...
                <span
                  className={`text-[10px] font-black px-1.5 py-0.5 rounded border shrink-0 ${
                    isMultistream
                      ? 'bg-fuchsia-950 text-fuchsia-200 border-fuchsia-500/50 shadow-[0_0_8px_rgba(217,70,239,0.3)]'
                      : isTwitch
              ...
              >
                {/* Multistream glowing badge in preview */}
                {isMultistream && (
                  <div className="absolute top-2 right-2 select-none pointer-events-none z-10">
                    <span className="text-[10px] drop-shadow-[0_0_6px_rgba(217,70,239,0.8)]">
                      🌐
                    </span>
                  </div>
                )}
                ...
                <div className="flex items-center gap-1.5 mb-1.5">
                  <div
                    className={`w-4 h-4 shrink-0 ${
                      isMultistream
                        ? 'rounded-full ring-1 ring-fuchsia-400 bg-fuchsia-500 shadow-[0_0_6px_rgba(217,70,239,0.8)]'
                        : isTwitch
                  ...
                  />
                  <div
                    className={`h-3 px-2 rounded-full flex items-center ${
                      isMultistream
                        ? 'bg-gradient-to-r from-fuchsia-600 to-pink-600 text-[8px] font-bold text-white'
                        : isTwitch
                  ...
                  {isMultistream && (
                    <div className="h-2.5 px-1.5 rounded-full bg-black/60 text-[7px] text-white/70 ml-1 font-mono">
                      07:40
                    </div>
                  )}
                  {isTwitch && (
                ...
                <div
                  className={`h-3 w-full rounded-full px-2 flex items-center mb-1 ${
                    isMultistream
                      ? 'bg-slate-950/85 border border-fuchsia-500/40 text-[8px] text-white/90'
                      : isTwitch
                      ? 'bg-neutral-200/80'
                      : isComic
                ...
                >
                  {isMultistream ? 'multistream message !!' : ''}
                </div>
                {!isMultistream && (
                  <div
                ...
                {isSelected ? (
                  <span
                    className={`w-5 h-5 rounded-full text-slate-950 flex items-center justify-center shrink-0 ml-1.5 ${
                      isMultistream
                        ? 'bg-gradient-to-r from-fuchsia-500 to-pink-500 text-white shadow-[0_0_10px_rgba(217,70,239,0.7)]'
                        : isTwitch
